...
public class GoozmaCordTargetContent : ARenderTargetContentByRequest
{
    ...
    protected override void HandleUseReqest(GraphicsDevice device, SpriteBatch spriteBatch)
    {
        ...
        for (var i = 0; i < partitions; i++) {
            var offset = new Vector2(20 + Utils.GetLerpValue(0, partitions / 2.1f, i, true) * Utils.GetLerpValue(partitions * 1.3f, partitions / 3f, i, true) * (140 + (float)Math.Sin((Host.NPC.localAI[0] * 0.125f - i / (partitions * 0.036f)) % MathHelper.TwoPi) * 18 * (i / partitions)), 0).RotatedBy(MathHelper.SmoothStep(0.15f, -3.3f, i / partitions));
