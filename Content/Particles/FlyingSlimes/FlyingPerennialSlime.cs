...
{
    public class FlyingPerennialSlime : Particle
    {
        public int time;
        ...
        public override void Update()
        {
            if (data is Vector2)
            {
                velocity = Vector2.Lerp(velocity, position.DirectionTo((Vector2)data).SafeNormalize(Vector2.Zero) * 24f, 0.1f);
            ...
            {
                Dust slime = Dust.NewDustPerfect(position + Main.rand.NextVector2Circular(20, 20), DustID.Crimson, velocity * 0.5f, 200, color, 0.5f + Main.rand.NextFloat());
