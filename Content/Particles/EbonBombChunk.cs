    ...
    public class EbonBombChunk : Particle
    {
        ...
        public override void Update()
        {
            ...
            if (!stuck)
            {
                ...
                if (Collision.SolidTiles(position - new Vector2(0, 12) * scale + velocity * 2f, 4, 4) && time > 20)
                {
                    if (!Main.dedServ)
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < 8; i++)
                            Dust.NewDustPerfect(position + Main.rand.NextVector2Circular(4, 4), DustID.TintableDust, -velocity * 0.1f + Main.rand.NextVector2Circular(2, 2), 200, Color.BlueViolet, Main.rand.NextFloat(2f) * scale);
