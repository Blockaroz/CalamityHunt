...
namespace CalamityHunt.Content.NPCs
{
    public class GelatinousSpawn : ModNPC, ISubjectOfNPC<Goozma>
    {
        ...
        public override void AI()
        {
            ...
            if (Time > 800) {
                for (var i = 0; i < (int)((Time - 400) / 800f) + 1; i++) {
            ...
            for (var i = 0; i < (int)(Time / 1000f) + 1; i++) {
                CalamityHunt.particles.Add(Particle.Create<ChromaticEnergyDust>(particle => {
            ...
            if (Time == 650) {
                var intro = AssetDirectory.Sounds.Goozma.Intro;
            ...
            if (Time > 1056) {
                for (var i = 0; i < 3; i++) {
                    var velocity = Vector2.UnitY.RotatedBy(MathHelper.TwoPi / 3f * i).RotatedByRandom(1f);
        ...
        private void FadeMusicOut(On_Main.orig_UpdateAudio orig, Main self)
        {
            ...
            if (Main.npc.Any(n => n.active && n.type == ModContent.NPCType<GelatinousSpawn>())) {
                var goozma = Main.projectile.FirstOrDefault(n => n.active && n.type == ModContent.NPCType<GelatinousSpawn>());
                for (var i = 0; i < Main.musicFade.Length; i++) {
                    var volume = Main.musicFade[i] * Main.musicVolume * Utils.GetLerpValue(300, 60, goozma.ai[0], true);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            var eye = AssetDirectory.Textures.Goozma.GodEye.Value;
            ...
            var eyePos = NPC.Center + drawOffset + new Vector2(-42, -37).RotatedBy(NPC.rotation) * NPC.scale;
            var eyeScale = (float)Math.Sqrt(Utils.GetLerpValue(940, 950, Time, true)) * 3f;
            var eyeRot = (float)Math.Cbrt(Utils.GetLerpValue(940, 1080, Time, true)) * MathHelper.PiOver2 - MathHelper.PiOver4;
