	...
	public class GoozmagaBomb : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (speedGate)
			{
				...
				Particle.NewParticle(ModContent.GetInstance<HueLightDust>(), Projectile.Center, Vector2.Zero, color, 1f);
			}
			else
			{
					...
					for (int i = 0; i < Main.maxProjectiles; i++)
					{
						...
						if (proj.type == ModContent.ProjectileType<GoozmagaBomb>() && proj.whoAmI != Projectile.whoAmI && proj.ai[0] == target.whoAmI + 1 && proj.active)
						{
							...
							for (int s = 0; s < 20; s++)
							{
								...
								Particle.NewParticle(ModContent.GetInstance<HueLightDust>(), inward, -inward.DirectionTo(Projectile.Center) * Main.rand.NextFloat(3f), color, 1f);
							}
							Projectile.Kill();
							break;
						}
